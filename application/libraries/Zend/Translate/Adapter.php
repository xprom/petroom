 ...
 * @version    $Id: Adapter.php 24594 2012-01-05 21:27:01Z matthew $
 */
...
abstract class Zend_Translate_Adapter {
        ...
        if (is_string($options['content']) and is_dir($options['content'])) {
            ...
            foreach ($iterator as $directory => $info) {
                ...
                if ($info->isDir()) {
                    ...
                    // @MOD
                    if( $file == 'custom.csv') {
                      $custom_language_array[] = $options['locale'];                                            
                    }
                    // END OF MOD
            ...
            // @MOD Add Custom.csv Last
            if( !empty( $custom_language_array) ){
              foreach( $custom_language_array as $custom_locale){
                $options['locale'] = $custom_locale;
                $options['content'] = APPLICATION_PATH . DIRECTORY_SEPARATOR . 'application' . DIRECTORY_SEPARATOR .
                                                    'languages' . DIRECTORY_SEPARATOR . $custom_locale .
                                                     DIRECTORY_SEPARATOR . 'custom.csv';
                $this->_addTranslationData($options);
              }
            }  
            // END OF MOD
            ...
            if (!Zend_Locale::isLocale($locale, false, false)) {
                ...
                if (!empty($this->_options['route'])) {
                    if (array_key_exists($locale, $this->_options['route']) &&
                        ...
                        return stripslashes($this->translate($messageId, $this->_options['route'][$locale]));
                    }
                }

                $this->_routed = array();
                if ($plural === null) {
                    return stripslashes($messageId);
                }
                ...
                return stripslashes($plural[$rule]);
            }
        ...
        if ((is_string($messageId) || is_int($messageId)) && isset($this->_translate[$locale][$messageId])) {
            ...
            if ($plural === null) {
                ...
                if(!is_array($this->_translate[$locale][$messageId])){
                  return stripslashes($this->_translate[$locale][$messageId]);
                }
                else{
            ...
            if (isset($this->_translate[$locale][$plural[0]][$rule])) {
                $this->_routed = array();
                return stripslashes($this->_translate[$locale][$plural[0]][$rule]);
            }
    ...
    {
      // Added Strip Slashes for PHP Array Translation Adapter
      return stripslashes($this->translate($messageId, $locale));
    }
